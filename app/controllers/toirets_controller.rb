class ToiretsController < ApplicationController
  ...
  def situation
    @toiret_maxdate_all = Toiret.group(:toiret_floor,:toiret_num).select("toiret_floor,toiret_num,flag,used_time,MAX(datetime) as datetime")
  ...
  def meeting
    ...
    @toiret_vlist = []
      ...
      @vtoiret = Vtoiret.new
      @vtoiret.toiret_floor = "10"
      toiret1 = @toiret_maxdate_group.where(toiret_floor: @vtoiret.toiret_floor,toiret_num: '0')
      toiret1.each do |t1|
        @vtoiret.toiret_num1 = 0
        @vtoiret.flag1 = t1.flag
        @vtoiret.datetime1 = t1.datetime
